...
class ComputePropPowerReq(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('rho', val= 1 * np.ones(self.options['n']), desc='air density', units='kg/m**3')
        self.add_input('thrust_total', val= 1 * np.ones(self.options['n']), desc='total aircraft thrust required', units='N')
        self.add_input('n_motors', val=2 , desc='number of engines', units=None)
        self.add_input('vel', val= 1 * np.ones(self.options['n']), desc='true airspeed', units='m/s')
        self.add_input('eta_prop', val=1 , desc='propeller efficiency', units=None) 

        # Outputs
        self.add_output('shaft_power_unit', val= 1 * np.ones(self.options['n']), desc='power required per engine', units='W')
    ...
    def compute_partials(self, inputs, J):  
        ...
        J['shaft_power_unit', 'thrust_total'] = np.eye(n) * (vel + 1.5 * thrust_total**0.5 / np.sqrt(2 * rho * diskarea)) / n_motors / eta_prop / eta_prplsv
        J['shaft_power_unit', 'vel'] = np.eye(n) * thrust_unit / eta_prop / eta_prplsv
        ...
        J['shaft_power_unit', 'rho'] = np.eye(n) * thrust_unit ** 1.5 * (-0.5) *  (2 * rho *diskarea)**(-1.5) * ( 2 * diskarea) / eta_prop / eta_prplsv
...
if __name__ == "__main__":
    ...
    model.add_subsystem('ComputePropPowerReq', ComputePropPowerReq(), promotes_inputs=['*'])
    ...
    print('shaft_power_unit = ', p['ComputePropPowerReq.shaft_power_unit']/1000, 'kW')
